SYNTH MMR-HISTORY "MMR - History of measles, mumps and rubella" IS ALL OF
  COND:C146 "History of measles" = true
  COND:C148 "History of rubella" = true
  COND:C147 "History of mumps infection" = true
	
CALC Meas_doses_received IS HIST(MEAS,0,count)
CALC Meas_age_at_first_dose IS INTERVAL(BASE:dob,HIST(MEAS,1,date))

CALC Meas_last_dose_date IS HIST(MEAS,-1,date)
CALC Meas_time_since_last_dose IS INTERVAL(CALC:Meas_last_dose_date,BASE:eval)

CALC Meas_d1d2 IS INTERVAL(HIST(MEAS,1,date),HIST(MEAS,2,date))
...
FOLDER 0 "Common to measles, mumps and rubella"
  ...
  FOLDER 1 "[12m-64y] old AND History of measles, mumps and rubella -"
    IF ALL OF
    SYNTH:MMR-HISTORY "MMR - History of measles, mumps and rubella" = false
    CALC:Age in 6m..64y

    FOLDER 11 "Zero dose"
      IF CALC:Meas_doses_received = 0
    ...
    FOLDER 12 "One dose"
      ...
      RULE 12/01 "[12m-7y] old, 1 dose: To do 6 to 8w after LD and from 7-8y old"
        ...
        DO Recommended
          Status DUE
          Age 7y..8y
          Delay 6w..8w from CALC:Meas_last_dose_date
    ...
    FOLDER 13 "Two doses"
      IF CALC:Meas_doses_received = 2

      RULE 13/01 "d1 ≥ 6m old, 2 doses: Up to date"
        IF CALC:Meas_age_at_first_dose>=6m
        DO Recommended
          Status COMPLETED
        MESSAGES  MSG11
      END RULE 13/01
      ...
      RULE 13/02 "d1 ≤ 5m old, 2 doses: To do 6-8w after LD"
        IF CALC:Meas_age_at_first_dose<=5m
        DO Recommended
          Status DUE
          Delay 6w..8w from CALC:Meas_last_dose_date
      END RULE 13/02
    ...
    FOLDER 14 "Three doses"
      IF CALC:Meas_doses_received = 3
    ...
    FOLDER 15 "Four doses or more"
      IF CALC:Meas_doses_received>=4
  ...
  FOLDER 9 "Further information"
    ...
    RULE 9/02 "d1 [6-8m] old, 2 doses: message d1 too early"
      IF ALL OF
        CALC:Meas_age_at_first_dose in 6m..8m
    ...
    RULE 9/06 "[13-50y] old, Fem, LD ≤ 31d: message contraception"
      IF ALL OF
        CALC:Meas_time_since_last_dose<=1m
...
FOLDER 1 "Only for measles"

  RULE 1/01 "History of measles+: message"
    WHEN Recommended
    IF ALL OF
      COND:C146 "History of measles" = true
      SYNTH:MMR-HISTORY "MMR - History of measles, mumps and rubella" = false
...
MESSAGE MSG20 Summary ANY PRIO 0
  "Despite a history of measles, vaccination with a trivalent vaccine is indicated."@en
 END MESSAGE MSG20
