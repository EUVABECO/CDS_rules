...
SYNTH MEN-W-RECEIVED "MEN-W- At least one dose of W vaccine" IS ANY OF
  CALC:MenW_received_valences contains MCV-W
  CALC:MenW_received_valences contains MPV-W

CALC MenW_doses_received IS HIST(MenW,0,count)
CALC MenW_received_valences IS HIST(MenW,0,valences)

TARGET MenW

FOLDER 0 "General rules"
...
FOLDER 1 "Common ACWY (Copie)"

  FOLDER 11 "≤ 23m old (Copie)"
    IF CALC:Age<=23m
  ...
  FOLDER 9 "Further informations (Copie)"
...
FOLDER 2 "Specific rules"
  ...
  RULE 2/01 "≥ 1 dose W, not ACWY: To do 1 dose 1m after LD with ACWY"
    IF ALL OF
      SYNTH:MEN-ACWY-CONJ "MEN-ACWY- At least one dose of ACWY conjugate vaccine" = false
      SYNTH:MEN-W-RECEIVED "MEN-W- At least one dose of W vaccine" = true
    DO Suggested
