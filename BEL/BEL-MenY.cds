SYNTH MEN-Y-RECEIVED "MEN-Y- At least one dose of Y vaccine" IS ANY OF
  CALC:MenY_received_valences contains MCV-Y
  CALC:MenY_received_valences contains MPV-Y
  
CALC MenY_doses_received IS HIST(MenY,0,count)
CALC MenY_received_valences IS HIST(MenY,0,valences)  
...
FOLDER 2 "Specific rules"
  ...
  RULE 2/01 "≥ 1 dose Y, not ACWY: To do 1 dose 1m after LD with ACWY"
    IF ALL OF
      SYNTH:MEN-ACWY-CONJ "MEN-ACWY- At least one dose of ACWY conjugate vaccine" = false
      SYNTH:MEN-Y-RECEIVED "MEN-Y- At least one dose of Y vaccine" = true
    DO Suggested
