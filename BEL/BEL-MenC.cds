SYNTH MEN-C-RECEIVED "MEN-C- At least one dose of C vaccine" IS ANY OF
  CALC:MenC_received_valences contains MCV-C
  CALC:MenC_received_valences contains MPV-C

CALC MenC_doses_received IS HIST(MenC,0,count)
CALC MenC_received_valences IS HIST(MenC,0,valences)
	
TARGET MenC
FOLDER 1 "Specific rules"

  RULE 1/01 "≥.1 dose C, not ACWY: To do 1 dose 1m after LD with ACWY"
    IF ALL OF
      SYNTH:MEN-ACWY-CONJ "MEN-ACWY- At least one dose of ACWY conjugate vaccine" = false
      SYNTH:MEN-C-RECEIVED "MEN-C- At least one dose of C vaccine" = true
    DO Suggested
