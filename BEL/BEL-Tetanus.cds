...
CALC T_first_dose_age IS INTERVAL(BASE:dob,HIST(T,1,date))

CALC T_last_dose_is_booster IS HIST(T,-1,booster)
CALC T_last_dose_date IS HIST(T,-1,date)
CALC T_last_dose_age IS INTERVAL(BASE:dob,CALC:T_last_dose_date)
CALC T_time_since_last_dose IS INTERVAL(CALC:T_last_dose_date,BASE:eval)

CALC T_interval_between_last_acts IS INTERVAL(HIST(T,-2,date),CALC:T_last_dose_date)
...
FOLDER 0 "Common DTaP"
  ...
  FOLDER 1 "[1m-17y] old"
    ...
    FOLDER 02 "Last dose is not a booster"
      ...
      FOLDER 026 "Six doses and more"
        IF CALC:T_doses_received>=6
    ...
    FOLDER 03 "Last dose is a booster and LD ≥ 5y old"
      IF ALL OF
      CALC:T_last_dose_is_booster = true
      CALC:T_last_dose_age>=5y

      RULE 03/01 "Booster+, LD ≤ 14y old: To do 15-16y old, 5y after LD"
        IF CALC:T_first_dose_age<=14y
        DO Recommended
          Status DUE
          Age 15y..16y
          Delay 5y from CALC:T_last_dose_date
  ...
  FOLDER 2 "≥ 18y old"
    ...
    FOLDER 22 "Last dose is not a booster"
      ...
      FOLDER 221 "One dose"
        ...
        RULE 221/01 "Booster-, 1 dose: To do from 4-6w after LD"
          ...
          DO Recommended
            Status DUE
            Age 3m
            Delay 4w..8w from CALC:T_last_dose_date
        END RULE 221/01
      END FOLDER 221
      ...
      FOLDER 222 "Two doses"
        ...
        RULE 222/01 "Booster-, 2 doses: To do 1y after LD"
          ...
          DO Recommended
            Status DUE
            Delay 12m from CALC:T_last_dose_date
        END RULE 222/01
      END FOLDER 222

      FOLDER 223 "Three doses or more"
        IF CALC:T_doses_received>=3

        RULE 223/01 "Booster-, ≥3 doses, PD-LD ≤ 19y, LD ≤ 19y: To do from 10y after LD"
          IF ALL OF
            CALC:Age>=18y
            CALC:T_time_since_last_dose<=19y
            CALC:T_interval_between_last_acts<=19y
          DO Recommended
            Status DUE
            Delay 10y from CALC:T_last_dose_date
          MESSAGES  MSG41
        ...
        RULE 223/02 "Booster-, ≥3 doses, LD ≥ 20y: To do ASAP"
          IF ALL OF
            CALC:Age>=18y
            CALC:T_time_since_last_dose>=20y
          DO Recommended
            Status DUE
            Age 18y
          MESSAGES  MSG40
        ...
        RULE 223/03 "Booster-, ≥3 doses, PD-LD ≥ 20y: To do 6m after LD"
          IF ALL OF
            CALC:Age>=18y
            CALC:T_interval_between_last_acts>=20y
          DO Recommended
            Status DUE
            Age 18y
            Delay 6m from CALC:T_last_dose_date
    ...
    FOLDER 23 "Last dose is a booster"
      IF CALC:T_last_dose_is_booster = true

      RULE 23/01 "Booster+, 1 dose, LD ≤ 19y: To do from 10y after LD"
        IF ALL OF
          CALC:T_doses_received = 1
          CALC:T_time_since_last_dose<=19y
        DO Recommended
          Status DUE
          Delay 10y from CALC:T_last_dose_date
