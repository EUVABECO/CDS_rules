...
FOLDER 0 "MMR"
  ...
  FOLDER 0 "History Rubella +"
    IF COND:C148 "History of rubella" = true

    RULE 0/01 "≥18y old, History Measles+: Up to date"
      IF CALC:Age>=18y
      DO Recommended
        Status COMPLETED
      MESSAGES  MSG147 MSG146
    END RULE 0/01
  END FOLDER 0

  FOLDER 1 "History Rubella -"
    IF COND:C148 "History of rubella" = false
    ...
    RULE 1/02 "≥18y old, 1 dose: To do 12m after LD"
      ...
      DO Recommended
        Status DUE
        Age 18y
        Delay 28d from CALC:Rub_last_dose_date
    END RULE 1/02
...
MESSAGE MSG147 Justification ANY PRIO 0
  "When a person is infected with <strong>rubella</strong>, they develop protective antibodies against this disease. Subsequently, this person is therefore protected and vaccination against rubella is not necessary. However, having had rubella does not contraindicate the trivalent measles-mumps-rubella (MMR) vaccination if it is otherwise recommended."@en
 END MESSAGE MSG147

MESSAGE MSG146 Summary ANY PRIO 0
  "Rubella: protection against this disease is provided if a history of rubella is confirmed."@en
 END MESSAGE MSG146
