CALC MenW_doses_received IS HIST(MenW,0,count)

CALC MenW_first_dose_date IS HIST(MenW,1,date)
CALC MenW_age_at_first_dose IS INTERVAL(BASE:dob,CALC:MenW_first_dose_date)
CALC MenW_first_vaccine IS HIST(MenW,1,vaccine)

CALC MenW_penultimate_dose_date IS HIST(MenW,-2,date)

CALC MenW_last_dose_date IS HIST(MenW,-1,date)
CALC MenW_age_at_last_dose IS INTERVAL(BASE:dob,CALC:MenW_last_dose_date)
CALC MenW_last_valences IS HIST(MenW,-1,valences)

CALC  MenW_d1d2 IS INTERVAL(CALC:MenW_first_dose_date,HIST(MenW,2,date))	
...
FOLDER 0 "Common AWY (Copie)"
  ...
  FOLDER 1 "RF+ HSCT- (Copie)"
    ...
    RULE 1/01 "RF+, LD = unconjugated : To do 2y after LD"
      ...
      DO Recommended
        Status DUE
        Delay 2y from CALC:MenW_last_dose_date
      MESSAGES  MSG237 MSG238
    END RULE 1/01

    FOLDER 11 "Zero dose (Copie)"
      IF CALC:MenW_doses_received = 0
    ...
    FOLDER 12 "One dose conjugated QV (Copie)"
      ...
      RULE 12/03 "≥ 12m old, RF+, HSCT-, 1 dose conjugated QV, d1 ≤ 11m: To do booster after 12m old, 2m after LD"
        IF ALL OF
          CALC:MenW_age_at_first_dose<=11m
          CALC:Age>=12m
        DO Recommended
          Status DUE
          Age 12m
      ...
      RULE 12/04 "≥ 12m old, RF+, HSCT-, 1 dose conjugated QV, d1 ≥ 12m old: To do booster 5y after LD"
        IF CALC:MenW_age_at_first_dose>=12m
        DO Recommended
          Status DUE
          Delay 5y from CALC:MenW_last_dose_date
        MESSAGES  MSG252
      END RULE 12/04
    END FOLDER 12
    ...
    FOLDER 13 "Two doses  or more conjugated QV (Copie)"
      ...
      RULE 13/02 " RF+, HSCT-, ≥ 2 doses, LD is conjugated QV, LD ≥ 12m old: To do booster 5y after LD"
        IF CALC:MenW_age_at_last_dose>=12m
        DO Recommended
          Status DUE
          Delay 5y from CALC:MenW_last_dose_date
        MESSAGES  MSG266
  ...
  FOLDER 2 "HSCT+ (Copie)"
    ...
    RULE 2/01 "Message to HSCT concerning the invalidity of previous doses, ≥ 1 dose, diff d1-HSCT ≥ 1d"
      IF ALL OF
        ...
        CALC:Age>=6w
        INTERVAL(CALC:MenW_first_dose_date,COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)")>=1d
      DO Neutral
      MESSAGES  MSG250 MSG251
    END RULE 2/01

    RULE 2/02 "Message to HSCT if LD administered less than 12 months after HSCT"
      IF INTERVAL(COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)",CALC:MenW_last_dose_date) in 1d..11m
      DO Recommended
        Status DUE
        Age 12m
        Delay 6m from CALC:MenW_last_dose_date
      MESSAGES  MSG270
    END RULE 2/02
    ...
    FOLDER 22 "One dose or more (LD = conjugated) (Copie)"
      IF ALL OF
      CALC:MenW_doses_received>=1
      CALC:MenW_last_valences contains MCV-A

      FOLDER 221 "LD before HSCT (Copie)"
        IF INTERVAL(CALC:MenW_last_dose_date,COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)")>=1d

        RULE 221/01 "LD before HSCT: To do [12-18m] after HSCT (Pre-HSCT doses do not count)"
          IF CALC:MenW_doses_received>=1
          DO Recommended
            Status DUE
            Delay 12m..18m from COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)"
        END RULE 221/01
      ...
      FOLDER 222 "LD after HSCT (Copie)"
        IF INTERVAL(COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)",CALC:MenW_last_dose_date)>=1d

        RULE 222/01 "HSCT+, Only 1 dose after HSCT, 0 dose before, LD ≥ 4w after HSCT: To do after 12m old, 6m after LD"
          IF ALL OF
            CALC:MenW_doses_received = 1
            INTERVAL(COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)",CALC:MenW_last_dose_date)>=4w
          DO Recommended
            Status DUE
            Age 12m
            Delay 6m from CALC:MenW_last_dose_date
        ...
        RULE 222/02 "HSCT+, Only 1 dose after HSCT, ≥1 dose before, BLD before HSCT, LD ≥ 4w after HSCT: To do 6m after LD"
          IF ALL OF
            INTERVAL(CALC:MenW_penultimate_dose_date,COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)")>=1d
            INTERVAL(COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)",CALC:MenW_last_dose_date)>=4w
          DO Recommended
            Status DUE
            Delay 6m from CALC:MenW_last_dose_date
        ...
        RULE 222/03 "HSCT+, ≥ 2 doses after HSCT, BLD ≥ 4w after HSCT, BLD-LD ≥ 4w: To do booster 5y after LD"
          IF ALL OF
            CALC:Age>=12m
            CALC:MenW_doses_received>=2
            INTERVAL(COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)",CALC:MenW_penultimate_dose_date)>=4w
            INTERVAL(CALC:MenW_penultimate_dose_date,CALC:MenW_last_dose_date)>=4w
          DO Recommended
            Status DUE
            Delay 5y from CALC:MenW_last_dose_date
          MESSAGES  MSG252
        ...
        RULE 222/04 "HSCT+, ≥ 1 dose, BLD ≤ 3w after HSCT: Special case - vaccination too soon after HSCT"
          IF INTERVAL(COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)",CALC:MenW_penultimate_dose_date) in 1d..3w
          DO Exception
          MESSAGES  MSG289
        END RULE 222/04

        RULE 222/05 "HSCT+, ≥ 1 dose, LD ≤ 3w after HSCT: Special case - vaccination too soon after HSCT"
          IF INTERVAL(COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)",CALC:MenW_last_dose_date) in 1d..3w
          DO Exception
          MESSAGES  MSG292
        END RULE 222/05
      END FOLDER 222
  ...
  FOLDER 8 "Contraindications and special cases (Copie)"
    ...
    RULE 8/02 "First dose too early (≤ 5w)"
      IF CALC:MenW_age_at_first_dose<=5w
      DO Exception
      MESSAGES  MSG272
    END RULE 8/02
  ...
  FOLDER 9 "Further information (Copie)"
    ...
    FOLDER 91 "Common messages ACWY (Copie)"
      ...
      RULE 91/04 "1 dose MENVEO before 2y old: message not before 24m (Only for C)"
        IF ALL OF
          CALC:MenW_age_at_first_dose<=23m
          CALC:MenW_first_vaccine = VAC0141 #MENVEO
        DO Neutral
        MESSAGES  MSG258
      END RULE 91/04
