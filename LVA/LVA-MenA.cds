...
CALC MenA_first_dose_date IS HIST(MenA,1,date)
CALC MenA_last_dose_date IS HIST(MenA,-1,date)
CALC MenA_age_at_first_dose IS INTERVAL(BASE:dob,CALC:MenA_first_dose_date)
CALC MenA_age_at_last_dose IS INTERVAL(BASE:dob,CALC:MenA_last_dose_date)
CALC MenA_last_valence_ids IS HIST(MenA,-1,valences)
CALC MenA_penultimate_dose_date IS HIST(MenA,-2,date)
CALC MenA_first_vaccine IS HIST(MenA,1,vaccine)
CALC  MenA_d1d2 IS INTERVAL(HIST(MenA,1,date),HIST(MenA,2,date))
...
FOLDER 0 "Common AWY"
  ...
  FOLDER 1 "RF+ HSCT-"
    ...
    RULE 1/01 "RF+, LD = unconjugated : To do 2y after LD"
      ...
      DO Recommended
        Status DUE
        Delay 2y from CALC:MenA_last_dose_date
    ...
    FOLDER 12 "One dose conjugated QV"
      IF ALL OF
      ...
      RULE 12/01 "≤ 11m old, RF+, HSCT-, 1 dose conjugated QV, d1 [6w-5m]: To do 2m after LD"
        IF ALL OF
          CALC:MenA_age_at_first_dose in 6w..5m
          CALC:Age<=11m
        DO Recommended
          Status DUE
          Delay 2m from CALC:MenA_last_dose_date
        MESSAGES  MSG235
      END RULE 12/01

      RULE 12/02 "≤ 11m old, RF+, HSCT-, 1 dose conjugated QV, d1 [6-11m]: To do after 12m old, 2m after LD"
        IF ALL OF
          CALC:MenA_age_at_first_dose in 6m..11m
          CALC:Age<=11m
        DO Recommended
          Status DUE
          Age 12m
      ...
      RULE 12/03 "≥ 12m old, RF+, HSCT-, 1 dose conjugated QV, d1 ≤ 11m: To do booster after 12m old, 2m after LD"
        IF ALL OF
          CALC:MenA_age_at_first_dose<=11m
          CALC:Age>=12m
        DO Recommended
          Status DUE
          Age 12m
      ...
      RULE 12/04 "≥ 12m old, RF+, HSCT-, 1 dose conjugated QV, d1 ≥ 12m old: To do booster 5y after LD"
        IF CALC:MenA_age_at_first_dose>=12m
        DO Recommended
          Status DUE
          Delay 5y from CALC:MenA_last_dose_date
        MESSAGES  MSG252
      END RULE 12/04
    END FOLDER 12

    FOLDER 13 "Two doses  or more conjugated QV"
      IF ALL OF
      SYNTH:MEN-ACWY-Conj = true
      CALC:MenA_doses_received>=2

      RULE 13/01 " RF+, HSCT-, ≥ 2 doses, LD is conjugated QV, LD ≤ 11m old: To do booster after 12m old,  2m after LD"
        IF CALC:MenA_age_at_last_dose<=11m
        DO Recommended
          Status DUE
          Age 12m
          Delay 2m from CALC:MenA_last_dose_date
      ...
      RULE 13/02 " RF+, HSCT-, ≥ 2 doses, LD is conjugated QV, LD ≥ 12m old: To do booster 5y after LD"
        IF CALC:MenA_age_at_last_dose>=12m
        DO Recommended
          Status DUE
          Delay 5y from CALC:MenA_last_dose_date
        MESSAGES  MSG266
  ...
  FOLDER 2 "HSCT+"
    ...
    RULE 2/01 "Message to HSCT concerning the invalidity of previous doses, ≥ 1 dose, diff d1-HSCT ≥ 1d"
      IF ALL OF
        ...
        CALC:Age>=6w
        INTERVAL(CALC:MenA_first_dose_date,COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)")>=1d
      DO Neutral
      MESSAGES  MSG250 MSG251
    END RULE 2/01

    RULE 2/02 "Message to HSCT if LD administered less than 12 months after HSCT"
      IF INTERVAL(COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)",CALC:MenA_last_dose_date) in 1d..11m
      DO Recommended
        Status DUE
        Age 12m
        Delay 6m from CALC:MenA_last_dose_date
      MESSAGES  MSG270
    END RULE 2/02
    ...
    FOLDER 22 "One dose or more (LD = conjugated)"
      IF ALL OF
      CALC:MenA_doses_received>=1
      CALC:MenA_last_valence_ids  contains MCV-A

      FOLDER 221 "LD before HSCT"
        IF INTERVAL(CALC:MenA_last_dose_date,COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)")>=1d
      ...
      FOLDER 222 "LD after HSCT"
        IF INTERVAL(COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)",CALC:MenA_last_dose_date)>=1d

        RULE 222/01 "HSCT+, Only 1 dose after HSCT, 0 dose before, LD ≥ 4w after HSCT: To do after 12m old, 6m after LD"
          IF ALL OF
            CALC:MenA_doses_received = 1
            INTERVAL(COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)",CALC:MenA_last_dose_date)>=4w
          DO Recommended
            Status DUE
            Age 12m
            Delay 6m from CALC:MenA_last_dose_date
        ...
        RULE 222/02 "HSCT+, Only 1 dose after HSCT, ≥1 dose before, BLD before HSCT, LD ≥ 4w after HSCT: To do 6m after LD"
          IF ALL OF
            INTERVAL(CALC:MenA_penultimate_dose_date,COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)")>=1d
            INTERVAL(COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)",CALC:MenA_last_dose_date)>=4w
          DO Recommended
            Status DUE
            Delay 6m from CALC:MenA_last_dose_date
        ...
        RULE 222/03 "HSCT+, ≥ 2 doses after HSCT, BLD ≥ 4w after HSCT, BLD-LD ≥ 4w: To do booster 5y after LD"
          IF ALL OF
            CALC:Age>=12m
            CALC:MenA_doses_received>=2
            INTERVAL(COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)",CALC:MenA_penultimate_dose_date)>=4w
            INTERVAL(CALC:MenA_penultimate_dose_date,CALC:MenA_last_dose_date)>=4w
          DO Recommended
            Status DUE
            Delay 5y from CALC:MenA_last_dose_date
          MESSAGES  MSG252
        ...
        RULE 222/04 "HSCT+, ≥ 1 dose, BLD ≤ 3w after HSCT: Special case - vaccination too soon after HSCT"
          IF INTERVAL(COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)",CALC:MenA_penultimate_dose_date) in 1d..3w
          DO Exception
          MESSAGES  MSG289
        END RULE 222/04

        RULE 222/05 "HSCT+, ≥ 1 dose, LD ≤ 3w after HSCT: Special case - vaccination too soon after HSCT"
          IF INTERVAL(COND:C966 "Date of hematopoietic stem cell transplant (marrow transplant)",CALC:MenA_last_dose_date) in 1d..3w
          DO Exception
          MESSAGES  MSG292
        END RULE 222/05
      END FOLDER 222
  ...
  FOLDER 8 "Contraindications and special cases"
    ...
    RULE 8/02 "First dose too early (≤ 5w)"
      IF CALC:MenA_age_at_first_dose<=5w
      DO Exception
      MESSAGES  MSG272
    END RULE 8/02
  ...
  FOLDER 9 "Further information"
    ...
    FOLDER 91 "Common messages ACWY"
      ...
      RULE 91/04 "1 dose MENVEO before 2y old: message not before 24m (Only for C)"
        IF ALL OF
          CALC:MenA_age_at_first_dose<=23m
          CALC:MenA_first_vaccine = VAC0141 # MENVEO
        DO Neutral
        MESSAGES  MSG258
      END RULE 91/04
    ...
    FOLDER 92 "Usable vaccines"

      FOLDER 921 "HSCT- (Je ne sais pas trop ce qu'il faut conserver...)"
